"""
Credit Risk Default Gemini prompt templates.

OPTIONAL MODULE — remove if not using Gemini integration.

All prompts live here so Bintou Ba, Marienne Dosso, Lynda Allepo can iterate on prompt quality
...
Rules for all prompts in this file:
  ...
  - Write for Credit Analyst — someone who needs to act, not a data scientist
...
def explanation_prompt(record: dict, shap_df) -> str:
    ...
    TODO: replace the placeholder fields with the actual fields for Credit Risk Default.
    Include the most informative fields for Credit Analyst and the top SHAP features.
    ...
    # TODO: update this prompt for Credit Risk Default
    return f"""You are an assistant helping a Credit Analyst at NorthBay Bank TBD. A model has flagged the following record. Write 2 to 3 sentences explaining why in plain language the analyst can act on immediately. Do not mention model scores, probabilities, or SHAP values.
...
def system_prompt_chat(record_context: dict, related_history: list[dict]) -> str:
    ...
    TODO: update with the fields and history format for Credit Risk Default.
    """
    history_lines = "\n".join([
        f"  {i+1}. {str(r)[:120]}"
    ...
    # TODO: update this prompt for Credit Risk Default
    return f"""You are an assistant for a Credit Analyst at NorthBay Bank TBD. Answer questions about the record under review concisely and factually. If you cannot answer from available data, say so clearly.
...
def pattern_detection_prompt(records: list[dict]) -> str:
    ...
    in Credit Risk Default. Focus on what a senior Credit Analyst would notice
...
def report_prompt(metrics: dict, patterns: str, date_range: str) -> str:
    ...
    TODO: update metric keys and the report framing for Credit Risk Default.
    The report should answer: what did the system accomplish for NorthBay Bank TBD?
    """
    # TODO: update metric references for Credit Risk Default
...
Key metrics:
  TODO: add metrics relevant to Credit Risk Default
